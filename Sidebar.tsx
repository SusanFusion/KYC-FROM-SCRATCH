  ...
  return (
    <div className="flex h-full w-64 flex-col border-r border-border bg-card">
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto p-3">
        {NAV_ITEMS.filter((item) => item.href !== "/import" || user?.role === "lead").map((item) => {
      ...
      <div className="space-y-3 border-t border-border p-4">
        {user ? (
          ...
          <Link
            href="/login"
            onClick={onNavigate}
            className="block rounded-md bg-muted px-3 py-2 text-xs font-medium text-muted-foreground hover:text-foreground"
          >
            Lead / Manager sign in →
          </Link>
        )}
