...
import { RecordUnlockGate } from "@/components/scorecard/RecordUnlockGate";
...
export default async function AgentScorecardPage({ params }: { params: { agentId: string } }) {
  const [{ result, rank, dataset }, user, unlockedAgentIds] = await Promise.all([
  ...
  // QA Audit results AND this agent's specific penalty entries are
  // restricted: visible to Leads/Managers always, and to anyone else only
  // after entering the shared team password specifically for THIS agent
  // (see recordUnlock.ts) — general browsing needs no sign-in at all, but
  // this one section per agent still does. Every other metric is visible
  // to everyone regardless.
  const canSeeQaAudit = user?.role === "lead" || unlockedAgentIds.includes(params.agentId);
      ...
      <PageShell>
        ...
        <Card className="mt-4">
          ...
          <CardContent>
            ...
            {!canSeeQaAudit && (
              <div className="mt-3">
                <RecordUnlockGate agentId={agent.id} agentName={agent.name} />
              </div>
            )}
          </CardContent>
        </Card>
